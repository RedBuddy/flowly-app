...
export function GoalCard({ id, name, target, current, deadline }: Goal) {
  ...
  return (
    <div className="group relative bg-card rounded-2xl p-5 shadow-soft border border-border/50 hover:shadow-card transition-all duration-300">
      <div className="flex items-start justify-between gap-2 mb-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-baseline gap-2">
            ...
            {deadline && <span className="text-xs text-muted-foreground whitespace-nowrap flex-shrink-0">{formatDateTime(deadline)}</span>}
          </div>
        </div>
      ...
      {/* Quick action */}
      <Button size="sm" className="w-full rounded-xl" onClick={() => switchContributeModal(id, name)}>
